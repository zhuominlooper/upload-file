...
export default {
  ...
  mounted() {
    this.inputInstance = this.$refs["file-upload"];
    //获取选择文件的监听
  ...
  methods: {
    ...
    async uploadFile() {
      ...
      console.log(HashData, suffix);
      //获取服务器有没有该文件的断点分片
      this.alreadUploadFile =
        (await this.getChunkData(HashData)).fileList || [];
      this.alreadUploadFileCount = this.alreadUploadFile.length;
      //1.确定分片策略,拿到分片的数量和分片大小
      let maxChunkDataSize = 1024 * 100; //默认最大的分片大小(100k)
    ...
    monitorChooseUploadFile() {
      this.inputInstance.addEventListener("change", () => {
        ...
        if (!file || file.size < 1024 * 1024 * 5)
          return this.$message.warning("请选择至少5m的文件");
        //获取到文件
        this.fileData = file;
      });
    },
